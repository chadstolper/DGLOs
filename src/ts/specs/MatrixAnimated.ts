...
export class MatrixAnimated extends Technique {
	public draw(): void {
		...
		this.lib.setEdgeGlyphAttrs(new SVGAttrOpts("blue", "black", null, null, 500 / (this._library.data.timesteps[0].nodes.length - 1), 500 / (this._library.data.timesteps[0].nodes.length - 1), null));
