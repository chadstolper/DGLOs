	...
	public enableStepping() {
		...
		let prevButton = buttonDiv.append("button")
			...
			.on("click", function () {
				...
				if (!self._multipleTimestepsEnabled) {
					self.currentEdgeShape.draw(self._edgeGlyphMap.get(0).get(self.currentEdgeShape), self.data, self.timeStampIndex, _matrixAttrOpts); //TODO: change matrixattropts as needed?
				}
		...
		let nextButton = buttonDiv.append("button")
			...
			.on("click", function () {
				...
				if (!self._multipleTimestepsEnabled) {
					self.currentEdgeShape.draw(self._edgeGlyphMap.get(0).get(self.currentEdgeShape), self.data, self.timeStampIndex, _matrixAttrOpts);
				}
				if (!self._matrixViewEnabled) {
					self.runSimulation(true);
				}
